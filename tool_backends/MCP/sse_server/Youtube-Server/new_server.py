...
# 配置代理设置
proxies = {
    'http': 'http://172.17.0.1:5000',
    'https': 'http://172.17.0.1:5000'
}

# 为requests设置全局代理
import os
os.environ['HTTP_PROXY'] = 'http://172.17.0.1:5000'
os.environ['HTTPS_PROXY'] = 'http://172.17.0.1:5000'
...
class YouTubeService:
    ...
    def search_videos(self, query: str, max_results: int = 5, **options) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            resp=resp.get("video_results", [])[:max_results]
            # Extract only the first max_results video results
            return resp
    ...
    def get_video_comments(self, video_id: str, max_results: int = 10, **options) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            comments_sorting_token = GoogleSearch(params).get_dict().get("comments_sorting_token", [])
            top_token = None
            ...
            new_params={
                ...
                "v": video_id,
                "api_key": SERPAPI_KEY,
                "next_page_token":top_token
                }
            resp=GoogleSearch(new_params).get_dict().get("comments", [])[:max_results]
            return resp
        except HttpError as e:
            logger.error(f"Error getting comments: {e}")
            raise e
    
    def get_video_transcript(self, video_id: str, language: Optional[str] = 'en') -> List[Dict[str, Any]]:
        """
...
async def get_video_comments(
    ...
    """
    try:
        comments_data = youtube_service.get_video_comments(video_id, max_results)
...
if __name__ == "__main__":
    ...
    svc = YouTubeService()
    result = svc.get_video_transcript("3JZ_D3ELwOQ", language="en")
    print(result)