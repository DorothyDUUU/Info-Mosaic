...
async def transcript_summary(
    ...
    try:
        ...
        # Get transcript data
        try:
            raw_transcript_data = await youtube_service.get_video_transcript(video_id, language)
...
class YouTubeService:
    ...
    def __init__(self):
        ...
        #     httplib2.socks.PROXY_TYPE_HTTP, '2.57.21.173', 5665, proxy_user = 'chzwyhjn', proxy_pass = 'erj17pepb85h'
        # ))
        http = httplib2.Http(proxy_info=httplib2.ProxyInfo(
            httplib2.socks.PROXY_TYPE_HTTP, '172.17.0.1', 5000
        ))
        self.youtube = build('youtube', 'v3', developerKey=YOUTUBE_API_KEY, http=http)
        

        # 配置YouTube Transcript API使用代理
        self.ytt_api = YouTubeTranscriptApi(
            proxy_config=WebshareProxyConfig(
                proxy_username="chzwyhjn-rotate",
                proxy_password="erj17pepb85h",
    ...
    async def _run_blocking(self, func, *args, **kwargs):
        """Run a blocking function in the thread pool and return its result."""
    ...
    async def _serpapi_get(self, params: dict) -> dict:
        """Helper: call SerpApi (blocking GoogleSearch.get_dict) in executor.
        Use SERPAPI_KEY from environment unless params already contains api_key.
        """
        def _call():
    ...
    async def search_videos(self, query: str, max_results: int = 5, **options) -> Dict[str, Any]:
        """
        Search for YouTube videos based on query and options
        Returns first 5 video results from the search
        """
        params = {
            "engine": "youtube",
            "search_query": query,
        ...
        # pass optional filters directly to SerpApi (gl, hl, etc.)
        resp = await self._serpapi_get(params)
        return resp.get("video_results", [])[:max_results]
    ...
    async def get_channel_details(self, channel_name: str) -> Dict[str, Any]:
        """
        ...
        resp = await self._serpapi_get(params)
        print("channel search resp:", resp)
        resp = resp.get("channel_results", [])
        if resp != []:
            resp = resp[0]
        return resp

    async def get_video_comments(self, video_id: str, max_results: int = 10, **options) -> Dict[str, Any]:
        """
        ...
        # 先 await 得到 dict，然后再 .get
        initial_resp = await self._serpapi_get(params)
        comments_sorting_token = initial_resp.get("comments_sorting_token", [])
        
        top_token = None
        for item in comments_sorting_token:
            if item.get("title") == "Top comments":
                top_token = item.get("token")
                break
        
        new_params = dict(params)
        if top_token:
            new_params["next_page_token"] = top_token
        
        full_resp = await self._serpapi_get(new_params)
        return full_resp.get("comments", [])[:max_results]

    async def get_video_transcript(self, video_id: str, language: Optional[str] = 'en') -> List[Dict[str, Any]]:
        """
        Get transcript for a specific YouTube video (async wrapper).
        Runs youtube_transcript_api calls in the thread pool so it can be awaited.
        """
        video_id = self.parse_url(video_id)
        ...
        def _fetch():
            try:
                if language:
                    ...
                    return self.ytt_api.get_transcript(video_id)
            except (TranscriptsDisabled, NoTranscriptFound) as e:
                # Return empty list when no transcript available
        ...
        # run in threadpool and await result
        return await self._run_blocking(_fetch)


    async def get_related_videos(self, video_id: str, max_results: Optional[int] = 10) -> Dict[str, Any]:
        """
        ...
        resp = await self._serpapi_get(params)
        related=resp.get("related_videos", [])[:max_results]
        if related ==[]:
            return {'message': 'No related videos found for video ID'}
        return related
    ...
    async def get_video_enhanced_transcript(self, video_ids: List[str], options: Dict[str, Any]) -> Dict[str, Any]:
        """
            ...
            try:
                ...
                if include_metadata:
                    ...
                    video_result["metadata"] = {
                        ...
                        'publishedAt': video.get('published_date'),
                    }
                
                # Call the get_video_transcript method which returns transcript data
                raw_transcript_data = await self.get_video_transcript(video_id, language)
...
async def get_channel_resource(channel_id: str) -> Dict[str, Any]:
    ...
    """
    try:
        channel_data = await youtube_service.get_channel_details(channel_id)
...
async def search_videos(
    ...
    try:
        ...
        for item in search_results:
            video_id = item.get('link').split('v=')[-1]
...
async def get_video_details(video_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        details = {
            'id': video_id,
            'title': video.get('title'),
            'description': video.get('description',{}).get('content'),
            'publishedAt': video.get('published_date'),
            'channelTitle': video.get('channel', {}).get('name'),
            'channelSubscribers': video.get('channel', {}).get('subscribers'),
            'viewCount': video.get('views'),
            'likeCount': video.get('likes'),
            'chaptercount': len(video.get('chapters', {})),
...
async def get_channel_details(channel_name: str) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for item in comments_data:
            formatted_comments.append({ 
                ...
                'channelname':item.get('channel',{}).get('name'),
        ...
        return {
            ...
            'totalResults': len(formatted_comments),
        }
    except Exception as e:
...
async def get_video_transcript(video_id: str, language: Optional[str] = 'en') -> Dict[str, Any]:
    try:
        # 关键：必须 await 所有 async 方法
        video_data = await youtube_service.get_video_details(video_id)
        # 防御：如果没有 items 或结构不同，处理为空
        # SerpApi 返回的结构可能不是 youtube Data API 格式 — 根据你 get_video_details 的实现调整
        video = None
        if isinstance(video_data, dict):
            # SerpApi 返回的结构，尝试取第一项或适配你的格式
            if 'items' in video_data and video_data['items']:
                video = video_data['items'][0]
            else:
                # 如果你的 get_video_details 返回的是扁平结构（serpapi），直接用它
                video = video_data

        if not video:
            return {'error': f'Video {video_id} not found.', 'videoId': video_id}

        # await transcript (注意：现在是 async)
        transcript_data = await youtube_service.get_video_transcript(video_id, language)

        # 确保 transcript_data 是 list
        if not isinstance(transcript_data, list):
            # 可能发生错误或 SerpApi 格式差异
            return {
                'error': 'Transcript fetch returned unexpected format',
                'videoId': video_id,
                'raw': transcript_data
            }
        ...
        max_length = 2000
        for segment in transcript_data:
            # FetchedTranscriptSnippet 可能是 dict 或对象，使用 getattr 安全读取
            text = getattr(segment, 'text', None) or segment.get('text', '') if isinstance(segment, dict) else getattr(segment, 'text', '')
            start = getattr(segment, 'start', None) or (segment.get('start', 0) if isinstance(segment, dict) else 0)
            duration = getattr(segment, 'duration', None) or (segment.get('duration', 0) if isinstance(segment, dict) else 0)
        ...
        # Prepare metadata (根据 video 结构调整字段取值)
        metadata = {
            'videoId': video.get('id') if isinstance(video, dict) else video_id,
            'title': (video.get('snippet', {}) or {}).get('title') if isinstance(video, dict) else video.get('title', ''),
            'channelTitle': (video.get('snippet', {}) or {}).get('channelTitle') if isinstance(video, dict) else video.get('channelTitle', ''),
            'language': language or 'default',
            'segmentCount': len(formatted_transcript),
            'long': total_length
        }
        ...
        logger.exception(f"Error in get_video_transcript tool: {e}")
        return {'error': str(e)}
...
async def get_related_videos(video_id: str, max_results: Optional[int] = 10) -> Dict[str, Any]:
    ...
    try:
        ...
        if related_data == {'message': 'No related videos found for video ID'}:
            return related_data
        # Format the response
        formatted_videos = []
        for item in related_data:
            related_video_id = item.get('video_id')
...
async def get_video_enhanced_transcript(
    ...
    try:
        ...
        transcript = await youtube_service.get_video_enhanced_transcript(video_ids, options)
...
if __name__ == "__main__":
    ...
    svc = YouTubeService()
    result = svc.get_channel_details("PewDiePie")
    print(result)