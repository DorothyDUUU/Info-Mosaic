...
port=os.environ.get("FASTMCP_PORT", 8007)
transport=os.environ.get("FASTMCP_TRANSPORT", "sse")  # stdio, streamable-http, sse
...
# 创建带代理配置的Google Maps客户端
gmaps = googlemaps.Client(key=google_maps_api_key, requests_kwargs={'proxies': proxies})
...
#-------------------
@mcp.tool()
async def find_place(input: str, input_type: str="textquery", fields: list=["place_id", "formatted_address", "name", "geometry", "types", "rating"]) -> str:
...
@mcp.tool()
async def place_nearby(location: dict, radius: int, place_type: str) -> str:
...
@mcp.tool()
async def place_details(place_id: str, fields: list=["name", "formatted_address", "formatted_phone_number", "website", "types", "rating", "user_ratings_total"]) -> str:
    """
    Args:
