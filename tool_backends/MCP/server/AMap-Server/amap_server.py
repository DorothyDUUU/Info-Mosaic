...
def maps_ip_location(ip: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if data["status"] != "1":
            return {
                "error": f"IP Location failed: {data.get('info') or data.get('infocode')}"
            }
...
def maps_weather(city: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"city": forecasts[0]["city"], "forecasts": forecasts[0]["casts"]}
    except requests.exceptions.RequestException as e:
        return {"error": f"Request failed: {str(e)}"}
...
def maps_bicycling_by_address(
    ...
    try:
        ...
        if not origin_location:
            return {
                "error": "Could not extract coordinates from origin geocoding result"
            }
        ...
        route_result = maps_bicycling_by_coordinates(
...
@mcp.tool()
def maps_bicycling_by_coordinates(
...
def maps_direction_walking_by_address(
    ...
    try:
        ...
        route_result = maps_direction_walking_by_coordinates(
            origin_location, destination_location
        )
...
@mcp.tool()
def maps_direction_walking_by_coordinates(
...
def maps_direction_driving_by_address(
    ...
    try:
        ...
        route_result = maps_direction_driving_by_coordinates(
            origin_location, destination_location
        )
...
@mcp.tool()
def maps_direction_driving_by_coordinates(
    origin: str, destination: str
) -> Dict[str, Any]:
...
def maps_direction_transit_integrated_by_address(
    ...
    try:
        ...
        route_result = maps_direction_transit_integrated_by_coordinates(
...
def maps_distance(origins: str, destination: str, type: str = "1") -> Dict[str, Any]:
    ...
    try:
        ...
        if data["status"] != "1":
            return {
                "error": f"Direction Distance failed: {data.get('info') or data.get('infocode')}"
            }
...
def maps_text_search(
    keywords: str, city: str = "", citylimit: str = "false"
) -> Dict[str, Any]:
...
def maps_search_detail(id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if data["status"] != "1":
            return {
                "error": f"Get poi detail failed: {data.get('info') or data.get('infocode')}"
            }
