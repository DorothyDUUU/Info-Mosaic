...
例如，我希望创建一个新的 client，文件名为 `./MCP/server/hello/hello.py`。

```python
@mcp.tool()
def hello_world() -> str:
    # add your own logic and tools here!
    return "Hello world!"
```

文件定义好之后需要在 `./MCP/config/server_list.json` 中指定对应的py文件路径(在这里是`server/hello/hello.py`)，同时一些简单的mcp服务可以直接在mcp_server.py文件中定义，默认会从中导入
...
[
    ...
    "server/AMap-Server/amap_server.py",
