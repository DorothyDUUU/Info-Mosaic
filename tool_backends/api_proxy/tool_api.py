...
with open(os.path.join(current_dir, "/mnt/configs/mcp_config.json"), "r") as f:
    mcp_config = json.load(f)

base_url = mcp_config["tool_api_url"]


with open(os.path.join(current_dir, "/mnt/configs/web_agent.json"), "r") as f:
    config = json.load(f)
...
async def main():
    async with aiohttp.ClientSession() as session:
        result = await fetch_web_api(session, "https://www.baidu.com/")
        print(result)
